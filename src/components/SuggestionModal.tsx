        ...
        <div className="p-4 flex-1 flex flex-col gap-4 overflow-hidden">
          ...
          <div className="rounded-xl border p-3 bg-slate-50 space-y-3">
            <div className="flex flex-wrap items-center gap-3">
              ...
              <button
                ...
                className={`px-2 py-1 rounded border text-xs flex items-center gap-1 ${
                  showFilters ? "bg-slate-800 text-white" : "bg-white text-slate-700"
                }`}
              >
          ...
          {view === "list" ? (
            <ListView
              ...
              onAddToCompare={(item) => {
                if (!compareList.find(p => p.name === item.name)) {
...
function ListView({
  ...
  const filteredItems = items.filter(item => {
    const price = item.price?.length || 1;
    const rating = item.ratings?.google || 0;
    ...
    <div className="flex-1 overflow-y-auto border rounded-lg bg-white/60 divide-y">
      ...
      {!loading && filteredItems.length === 0 && !error && (
        <div className="text-sm text-slate-500 p-3">
          No matches. Try adjusting filters or removing area/near-me filter.
        </div>
      )}
