 ...
 * This version forces a re-fetch on slot open and shows
 * a spinner on the slot that is fetching. It also defaults
 * API_BASE to same-origin so you don't need env locally.
 ****************************************************/

// Resolve API base: window.__API_BASE__ → Vite env → same-origin
...
// SlotKey includes a special "hotel" slot so we can reuse the same chooser modal
type SlotKey =
...
type ApiSuggestion = {
  ...
  lat?: number;
  lng?: number;
  desc?: string; // description text (server may send `description`; we normalize to `desc`)
...
function buildDetailText(d: DayPlan | undefined, vibe: Vibe): string {
  ...
  const mealLine = (label: string, item?: SelectedItem) => {
    ...
    parts.push(
      `- ${label}: ${item.name} -> try ${dish.join(", ")} — ${VIBE_MEAL_TIPS[vibe]}`
    );
  };
    ...
    <div>
      <div className="flex items-center justify-between mb-2 text-xs">
        <div className="text-slate-500">
          {usingApi ? "Live data (AI)" : "API not configured"}
          {loading ? " · loading..." : ""}
        </div>
          ...
          >
            <div>
              ...
              <div className="text-[11px] text-slate-600 mt-0.5">
                {it.ratings?.combined != null ? (
                  <>★ {Number(it.ratings.combined).toFixed(1)} agg</>
                ) : null}
                {it.ratings?.yelp != null ? (
                  <> · Yelp {Number(it.ratings.yelp).toFixed(1)}</>
                ) : null}
                ...
                typeof it.ratings?.googleReviews === "number" ? (
                  <>
                    ...
                    ·{" "}
                    {(
                      (it.ratings?.yelpReviews || 0) +
                      (it.ratings?.googleReviews || 0)
                    ).toLocaleString()}{" "}
                    reviews
                  </>
                ) : null}
...
export default function PlannerRedesign() {
  ...
  // Force re-fetch when opening the same slot repeatedly
  ...
  function openSlot(slot: SlotKey) {
    ...
    setRefreshTick((t) => t + 1); // ensure a re-fetch even if filters didn't change
  }
  ...
  useEffect(() => {
    if (!usingApi) {
      ...
      setHotelError("API base not configured");
      return;
    }
  ...
  useEffect(() => {
    ...
    setLiveLoading(true);
    setLiveError(null);
    const url = `${API_BASE}/api/ai?${params.toString()}`;
    setLastFetchUrl(url);
    ...
    areaFilter,
    usingApi,
    refreshTick, // <— key addition: force re-fetch when slot is opened
  ]);

  // One-click rebuild for the current vibe
  async function rebuildDayForVibe(dayIndex1Based: number) {
    ...
    for (const cat of cats) {
      ...
      try {
        ...
        /* ignore per-slot errors so other slots can fill */
      }
    }
  }
  ...
  useEffect(() => {
    try {
      console.assert(
        Math.abs(haversineKm({ lat: 0, lng: 0 }, { lat: 0, lng: 0 })) < 1e-9,
        "haversine zero distance"
      );
      console.assert(etaMins("walk", 5) === 60, "eta walk 5km ~ 60m");
      const detailsEmpty = buildDetailText({}, "romantic");
      console.assert(
        detailsEmpty.includes("No selections"),
        "detail fallback when empty"
      );
      const detailsNL = buildDetailText(
        { breakfast: { name: "Cafe Example" }, dinner: { name: "Bistro Example" } },
        "romantic"
      );
      console.assert(detailsNL.includes("\n"), "detail joins with newline");
    ...
    <div className="min-h-screen bg-gradient-to-b from-sky-50 via-indigo-50 to-white p-6">
      <div className="max-w-6xl mx-auto space-y-5">
            ...
            {VIBES.map((v) => (
              <button
                ...
                className={`px-3 py-1.5 rounded-full border text-sm ${
                  vibe === v ? "bg-indigo-600 text-white border-indigo-600" : "bg-white text-slate-800"
                }`}
              >
            ...
            {(hotelLoading || hotelError || hotelSugs.length > 0) && (
              <div className="border rounded-xl p-2 bg-slate-50">
                <div className="text-xs text-slate-500 mb-1">
                  {hotelLoading ? "Searching…" : hotelError ? hotelError : "Matches"}
                </div>
        ...
        <div className="grid lg:grid-cols-2 gap-5">
          ...
          <div className="bg-white rounded-2xl border p-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <SlotButton
                ...
                loading={slotModalOpen && slotKey === "activity" && liveLoading}
              />
              <SlotButton
                label="Breakfast"
                value={cur.breakfast}
                onClick={() => openSlot("breakfast")}
                loading={slotModalOpen && slotKey === "breakfast" && liveLoading}
              />
              <SlotButton
                label="Lunch"
                value={cur.lunch}
                onClick={() => openSlot("lunch")}
                loading={slotModalOpen && slotKey === "lunch" && liveLoading}
              />
              <SlotButton
                label="Dinner"
                value={cur.dinner}
                onClick={() => openSlot("dinner")}
                loading={slotModalOpen && slotKey === "dinner" && liveLoading}
              />
              <SlotButton
                label="Coffee"
                value={cur.coffee}
                onClick={() => openSlot("coffee")}
                loading={slotModalOpen && slotKey === "coffee" && liveLoading}
              />
            </div>
            <textarea
              value={cur.notes || ""}
              onChange={(e) =>
                setPlan((prev) => {
                  const next = [...prev];
                  next[currentDay - 1] = { ...next[currentDay - 1], notes: e.target.value };
                  return next;
                })
              }
          ...
          <div className="bg-white rounded-2xl border p-4">
            ...
            <div className="text-[11px] text-slate-500 mb-2">
              Routes show real ETAs when the AI endpoint returns directions; otherwise
              straight-line estimates (walk ~5 km/h, drive ~35 km/h).
            </div>
                  ...
                  <g key={`rseg-${idx}`}>
                    ...
                    {(() => {
                      ...
                      const mx = (a.x + b.x) / 2,
                        my = (a.y + b.y) / 2;
                      return (
                        <text x={mx + 1} y={my - 1} fontSize={3} fill="#334155">
                          {seg.mins} min
                        </text>
                      );
                    })()}
                    ...
                    return (
                      <g key={`seg-${idx}`}>
                        <line
                          ...
                          strokeWidth={1.2}
                        />
                        <text x={midx + 1} y={midy - 1} fontSize={3} fill="#334155">
                          {seg.mins} min
                        </text>
                      </g>
                    );
        ...
        >
          ...
          <div className="text-[11px] text-slate-500 mb-2">
            <div>
              Last request URL: <span className="break-all">{lastFetchUrl || "(none yet)"}</span>
            </div>
        ...
        >
          <div className="space-y-3">
            ...
            <div className="text-[11px] text-slate-500">
              Suggestions are vibe-aware. Edit as you like.
            </div>
          </div>
        </Modal>
      </div>
