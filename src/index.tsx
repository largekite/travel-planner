 ...
 * - Remove duplicate/early API_BASE declaration (imports must come first)
 * - Make same-origin the default API base
 * - Fix newline strings ('\n') and remove non-ASCII class names
 * - Ensure geolocation handler `useMyLocation` exists in scope
 * - Add a debug readout for the last fetch URL and result count
 ****************************************************/

// Resolve API base from env/global, else fallback to same-origin
...
export default function PlannerRedesign() {
  ...
  // With same-origin fallback, we always have an API path available
  const usingApi = true;
  ...
  useEffect(() => {
    ...
    setLiveLoading(true); setLiveError(null); setLastFetchUrl(`${API_BASE}/api/ai?${params.toString()}`); setLastResultCount(0);
    fetch(`${API_BASE}/api/ai?${params.toString()}`, { signal: ctrl.signal })
      .then(async (r) => {
    ...
    return () => ctrl.abort();
  }, [slotModalOpen, city, vibe, slotKey, useNearFilter, nearMode, nearMaxMins, hotel?.lat, hotel?.lng, areaFilter]);
  ...
  async function rebuildDayForVibe(dayIndex1Based: number) {
    ...
    for (const cat of cats) {
      ...
      try {
        ...
        if (first) setSlot(dayIndex1Based, cat, { name: first.name, url: first.url, area: first.area, cuisine: first.cuisine, price: first.price, lat: first.lat, lng: first.lng, desc: first.desc, meta: first.meta });
      } catch {/* ignore per-slot errors so other slots can fill */}
    }
  }

  // Directions: ask the AI endpoint to compute real routes/ETAs when 2+ places are selected
  useEffect(() => {
    ...
    return () => ctrl.abort();
  }, [chosenItems.map(p=>p.name).join('|')]);

  // Print handler (final plan view)
  function handlePrint() { window.print(); }

  // Build text for detail modal on demand
  function openDetail(dayIdx1: number) { setDetailDay(dayIdx1); setDetailText(buildDetailText(plan[dayIdx1-1], vibe)); setDetailOpen(true); }
  function insertDetailIntoNotes() {
    setPlan((prev) => { const next = prev.map(d => ({...d})); const i = detailDay-1; const existing = next[i].notes?.trim(); next[i].notes = existing ? `${existing}\n\n${detailText}` : detailText; return next; });
    setDetailOpen(false);
  }
      ...
      <div className="max-w-6xl mx-auto space-y-5">
        ...
        <div className="rounded-2xl bg-white border p-4 flex flex-wrap items-center gap-3">
          ...
          <div className="flex items-center gap-2 text-sm">
            ...
            <input type="number" min={1} max={14} value={daysCount} onChange={(e)=>setDaysCount(Math.max(1, Math.min(14, parseInt(e.target.value||"1"))))} className="w-20 border rounded-lg p-1"/>
          </div>
          <div className="w-full sm:w-auto"><DaysStrip daysCount={daysCount} currentDay={currentDay} setCurrentDay={setCurrentDay}/></div>
        </div>
        ...
        <div className="rounded-2xl bg-white border p-4">
          ...
          <div className="flex flex-col gap-3">
            <div className="flex flex-wrap items-center gap-3">
              ...
              <button onClick={useMyLocation} className="px-3 py-2 rounded-lg border flex items-center gap-2 bg-white hover:bg-slate-50"><Crosshair className="w-4 h-4"/> Use my location</button>
              <button onClick={()=>openSlot('hotel')} className="px-3 py-2 rounded-lg border flex items-center gap-2 bg-white hover:bg-slate-50"><Hotel className="w-4 h-4"/> Open chooser</button>
            </div>
            {(hotelLoading || hotelError || hotelSugs.length>0) && (
              <div className="border rounded-xl p-2 bg-slate-50">
                ...
                <div className="divide-y max-h-60 overflow-auto">
                  {hotelSugs.map((h, i)=> (
                    <button key={`${h.name}-${i}`} onClick={()=>{ setHotel({ name: h.name, url: h.url, area: h.area, lat: h.lat, lng: h.lng, desc: h.desc }); setHotelQuery(h.name); }} className="w-full text-left py-2 px-2 hover:bg-white rounded-lg">
                      <div className="font-medium">{h.name}</div>
                      <div className="text-xs text-slate-600">{[h.area, h.desc].filter(Boolean).join(' · ')}</div>
                    </button>
                  ))}
                  {!hotelLoading && !hotelError && hotelSugs.length===0 && <div className="text-xs text-slate-500 py-2 px-2">No matches yet.</div>}
                </div>
              </div>
            )}
            <div className="flex flex-wrap items-center gap-3">
              {hotel?.name ? <div className="text-sm">Selected: <span className="font-medium">{hotel.name}</span>{hotel.area? <span className="text-slate-500"> · {hotel.area}</span>:null}</div> : <div className="text-sm text-slate-500">None selected</div>}
        ...
        <div className="grid lg:grid-cols-2 gap-5">
          ...
          <div className="bg-white rounded-2xl border p-4">
            <div className="flex items-center justify-between mb-2">
              ...
              <div className="flex items-center gap-2">
                <button onClick={()=>rebuildDayForVibe(currentDay)} disabled={!usingApi} className="text-xs flex items-center gap-1 px-2 py-1 rounded border bg-white hover:bg-slate-50 disabled:opacity-50">Rebuild for vibe</button>
                <button onClick={()=>openDetail(currentDay)} className="text-xs flex items-center gap-1 px-2 py-1 rounded border bg-indigo-50 hover:bg-indigo-100"><Wand2 className="w-3.5 h-3.5"/>Suggest details</button>
                <button onClick={()=>clearDay(currentDay)} className="text-xs text-slate-600 hover:text-slate-900 flex items-center gap-1"><Trash2 className="w-3.5 h-3.5"/>Clear</button>
              </div>
            </div>
            ...
            <textarea value={cur.notes || ""} onChange={(e)=>setPlan((prev)=>{ const next=[...prev]; next[currentDay-1]={...next[currentDay-1], notes: e.target.value}; return next; })} placeholder="Notes (times, confirmations, etc.)" className="mt-3 w-full border rounded-lg p-2 text-sm"/>
          </div>

          {/* Right: Map */}
          <div className="bg-white rounded-2xl border p-4">
            <div className="font-semibold mb-2">Map - Day {currentDay}</div>
            <div className="text-[11px] text-slate-500 mb-2">Routes show real ETAs when the AI endpoint returns directions; otherwise straight-line estimates (walk ~5 km/h, drive ~35 km/h).</div>
            <svg viewBox="0 0 100 100" className="w-full aspect-square rounded-xl border bg-slate-50">
              ...
              {dirSegs && dirSegs.map((seg, idx) => (
                <g key={`rseg-${idx}`}>
                  ...
                  {(() => { const first = seg.path[0]; const last = seg.path[seg.path.length-1]; if(!first||!last) return null; const a = project(first[0], first[1]); const b = project(last[0], last[1]); const mx=(a.x+b.x)/2, my=(a.y+b.y)/2; return <text x={mx+1} y={my-1} fontSize={3} fill="#334155">{seg.mins} min</text>; })()}
                </g>
              ))}
              ...
              {!dirSegs && (()=>{
                const segs: {a:SelectedItem,b:SelectedItem,km:number,mode:"walk"|"drive",mins:number}[] = [];
                for (let i=0;i<chosenItems.length-1;i++){ const A=chosenItems[i]!, B=chosenItems[i+1]!; if(!A.lat||!A.lng||!B.lat||!B.lng) continue; const km=haversineKm({lat:A.lat,lng:A.lng},{lat:B.lat,lng:B.lng}); const mode=km>1.2?'drive':'walk'; const mins=etaMins(mode,km); segs.push({a:A,b:B,km,mode,mins}); }
                return segs.map((seg, idx) => { const a = project(seg.a.lat, seg.a.lng); const b = project(seg.b.lat, seg.b.lng); const color = seg.mode === 'walk' ? '#16a34a' : '#2563eb'; const midx = (a.x + b.x) / 2; const midy = (a.y + b.y) / 2; return (
                  <g key={`seg-${idx}`}>
              ...
              {/* Pins for selections */}
              {chosenItems.map((p, idx) => { const { x, y, hidden } = project(p.lat, p.lng); if (hidden) return null; const hue = 220 + (idx*40); return (<g key={`${p.name}-${idx}`}><circle cx={x} cy={y} r={2.8} fill={`hsl(${hue} 80% 45%)`} /><text x={x+3.8} y={y+1.6} fontSize={3} fill="#334155">{p.name}</text></g>); })}

              {/* Hotel pin */}
              {hotel && hotel.lat && hotel.lng && (() => { const {x,y,hidden} = project(hotel.lat, hotel.lng); if (hidden) return null; return <g key="hotel"><rect x={x-2} y={y-2} width={4} height={4} fill="#f59e0b"/><text x={x+3.8} y={y+1.6} fontSize={3} fill="#92400e">Hotel/Center</text></g>; })()}
            </svg>
            <div className="mt-2 text-xs text-slate-600">{chosenItems.length === 0 ? (<div>No places selected yet. Choose slots on the left to plot and route them here.</div>) : (<ul className="list-disc pl-5">{chosenItems.map((p,i)=>(<li key={`${p.name}-legend-${i}`}>{p.name} {p.area? <span className="text-slate-500">· {p.area}</span> : null}</li>))}</ul>)}</div>
        ...
        {/* Plan View */}
        <div className="rounded-2xl bg-white border p-4">
          <div className="flex items-center justify-between"><div className="font-semibold">Plan View</div><div className="flex items-center gap-2"><button onClick={handlePrint} className="px-3 py-1.5 rounded-lg border bg-slate-50 hover:bg-slate-100 text-sm flex items-center gap-2"><Printer className="w-4 h-4"/>Print</button></div></div>
          <div className="mt-3 overflow-x-auto"><table className="min-w-full text-sm"><thead><tr className="text-left text-slate-500"><th className="py-2 pr-4">Day</th><th className="py-2 pr-4">Breakfast</th><th className="py-2 pr-4">Activity</th><th className="py-2 pr-4">Lunch</th><th className="py-2 pr-4">Coffee</th><th className="py-2 pr-4">Dinner</th><th className="py-2 pr-4">Notes</th></tr></thead><tbody>{plan.map((d, i) => (<tr key={`day-${i}`} className="border-t align-top"><td className="py-2 pr-4 font-medium whitespace-nowrap">Day {i+1}</td><td className="py-2 pr-4">{d.breakfast?.name || <span className="text-slate-400">—</span>}</td><td className="py-2 pr-4">{d.activity?.name  || <span className="text-slate-400">—</span>}</td><td className="py-2 pr-4">{d.lunch?.name     || <span className="text-slate-400">—</span>}</td><td className="py-2 pr-4">{d.coffee?.name    || <span className="text-slate-400">—</span>}</td><td className="py-2 pr-4">{d.dinner?.name    || <span className="text-slate-400">—</span>}</td><td className="py-2 pr-4 max-w-[320px] truncate" title={d.notes}>{d.notes || <span className="text-slate-400">—</span>}</td></tr>))}</tbody></table></div>
        </div>

        {/* Suggestions modal (also supports hotel when slotKey === 'hotel') */}
        <Modal open={slotModalOpen} title={`Choose ${slotKey === 'hotel' ? 'Hotel / Area' : slotKey.charAt(0).toUpperCase()+slotKey.slice(1)}`} onClose={()=>setSlotModalOpen(false)}>
          <div className="mb-4 rounded-xl border p-3 bg-slate-50">
            <div className="flex flex-wrap items-center gap-3">
              <div className="flex items-center gap-2 text-sm">
                <span className="text-slate-600">Area/Neighborhood</span>
                <input value={areaFilter} onChange={(e)=>setAreaFilter(e.target.value)} placeholder="e.g., Forest Park, Central West End" className="border rounded-lg p-1"/>
              </div>
              {slotKey !== 'hotel' && (
                <>
                  <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={useNearFilter} onChange={(e)=>setUseNearFilter(e.target.checked)} /><span>Filter by near me</span></label>
                  <div className="flex items-center gap-2 text-sm"><button onClick={()=>setNearMode('walk')} className={`px-2 py-1 rounded border text-xs ${nearMode==='walk'?"bg-emerald-600 text-white border-emerald-600":"bg-white"}`}><Footprints className="inline w-3.5 h-3.5 mr-1"/>Walk</button><button onClick={()=>setNearMode('drive')} className={`px-2 py-1 rounded border text-xs ${nearMode==='drive'?"bg-indigo-600 text-white border-indigo-600":"bg-white"}`}><Car className="inline w-3.5 h-3.5 mr-1"/>Drive</button></div>
                  ...
                  {!hotel && useNearFilter && (<div className="text-xs text-amber-700 bg-amber-50 border border-amber-200 px-2 py-1 rounded">Pick a hotel/center to enable near-me filtering.</div>)}
                </>
              )}
            </div>
          </div>
          {/* Debug: show exact request & result count to compare with manual URL */}
          <div className="text-[11px] text-slate-500 mb-2">
            <div>Last request URL: <span className="break-all">{lastFetchUrl || '(none yet)'}</span></div>
            <div>Items returned: {lastResultCount}</div>
          </div>
          <SuggestionList items={liveItems} onChoose={chooseForSlot} loading={liveLoading} error={liveError} usingApi={true}/>
        </Modal>

        {/* Detail modal */}
        <Modal open={detailOpen} title={`Suggested details - Day ${detailDay}`} onClose={()=>setDetailOpen(false)}>
          <div className="space-y-3">
            <textarea value={detailText} onChange={(e)=>setDetailText(e.target.value)} className="w-full border rounded-lg p-2 text-sm min-h-[160px]"/>
            <div className="flex items-center gap-2"><button onClick={insertDetailIntoNotes} className="px-3 py-1.5 rounded-lg border bg-indigo-50 hover:bg-indigo-100 text-sm">Insert into Day {detailDay} notes</button><button onClick={()=>setDetailOpen(false)} className="px-3 py-1.5 rounded-lg border bg-white hover:bg-slate-50 text-sm">Close</button></div>
